...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		if (isRoot())
		{
			throw std::logic_error("Node is a root, brother does not exist!");
		}
		else
		{
	...
	{
		*this->path_ = *(dynamic_cast<const TreeIterator&>(other).path_);
		return *this;
	}
	...
	{
		return *this->path_ != *(dynamic_cast<const TreeIterator&>(other).path_);
	}

	template<typename T>
	inline const T Tree<T>::TreeIterator::operator*()
	{
		TreeNode<T>* current = this->path_->front();
