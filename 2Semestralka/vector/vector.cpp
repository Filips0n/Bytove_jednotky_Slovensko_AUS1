...
namespace structures {
	...
	byte& Vector::readBytes(const int index, const int count, byte& dest)
	{
		...
		DSRoutines::rangeCheck(index + count, size_ + 1);
	...
	void Vector::copy(const Vector& src, const int srcStartIndex, Vector& dest, const int destStartIndex, const int length)
	{
		if (length != 0)
		{
			DSRoutines::rangeCheck(destStartIndex, dest.size_);
			DSRoutines::rangeCheck(srcStartIndex, src.size_);
			DSRoutines::rangeCheck(srcStartIndex + length, src.size_ + 1);
			DSRoutines::rangeCheck(destStartIndex + length, dest.size_ + 1);
