...
namespace structures
{
	...
	inline PriorityQueueList<T>& PriorityQueueList<T>::operator=(const PriorityQueueList<T>& other)
	{
		...
		{
			this->clear();
			for (PriorityQueueItem<T>* otherItem : *other.list_)
			{
				this->list_->add(new PriorityQueueItem<T>(*otherItem));
			}
		}
		return *this;
	}
	...
	inline int PriorityQueueList<T>::indexOfPeek() const
	{
		if (this->size() <= 0)
		{
			throw std::logic_error("Prazdny priority_queue_list");
		}
		int maxPriorita = (*this->list_)[0]->getPriority();
		int index = 0;
		int i = 0;
		for (PriorityQueueItem<T>* item : *this->list_) {
			if (item->getPriority() < maxPriorita)
			{
				maxPriorita = item->getPriority();
				index = i;
			}
			i++;
		}
	...
	{
		return (*this->list_)[this->indexOfPeek()]->accessData();
	}
	...
	{
		return (*this->list_)[this->indexOfPeek()]->getPriority();
	}
}